...
public class AssignedComplains extends JFrame {

	private JPanel contentPane;
	private JTable table;
	private PreparedStatement pscount,psdata,psdata1,psdata2;
	private ResultSet rscount,rsdata,rsdata1,rsdata2;
	private Connection cn;
	Object[][]data;
	String colnames[]={"complaintid","customerphone","customeraddress","productname","complainttext","dateofcmp"};
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					AssignedComplains frame = new AssignedComplains();
  ...
  public AssignedComplains() {
	...
	String strcount="select count(*) from complaintracking";
...
String strdata="select * from complaintracking where employeeid=?";
	psdata=cn.prepareStatement(strdata);
	
	System.out.println(psdata);
	psdata.setString(1, "tanish1");
	rsdata=psdata.executeQuery();
	int row=0;
	int flag=0;
	while(rsdata.next())
	{	flag=1;
		String x=rsdata.getString("complaintid");
		System.out.println(x);
		data[row][0]=rsdata.getString("complaintid");
		//data[row][1]=rsdata.getString("remarks");
	...
	String strs="select * from complaint where complaintid=?";
	//cn=psdata1.preparestat
	psdata1=cn.prepareStatement(strs);
	
	psdata1.setString(1, x);
	rsdata1=psdata1.executeQuery();
	while(rsdata1.next()) {
		String y=rsdata1.getString("customerid");
	data[row][3]=rsdata1.getString("productname");
	data[row][4]=rsdata1.getString("complaintext");
	data[row][5]=rsdata1.getString("dateofcmp");
	
	

	String strs1="select * from customer where customerid=?";
	//cn=psdata1.preparestat
	psdata2=cn.prepareStatement(strs1);
	
	psdata2.setString(1, y);
	rsdata2=psdata2.executeQuery();
	while(rsdata2.next()) {
	data[row][1]=rsdata2.getString("name");
	data[row][2]=rsdata2.getString("phoneno");
